...
export default function UploadPage() {
  ...
  const handleUrlSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_FASTAPI_URL}/api/upload/url`, {
        method: 'POST',
  ...
  const handleTextSubmit = async (e: React.FormEvent) => {
    ...
    setIsProcessing(true)
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_FASTAPI_URL}/api/upload/text`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          type: 'text',
